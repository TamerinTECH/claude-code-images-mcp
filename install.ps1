...
if (Test-Path ".env") {
    ...
    if ($choice -eq "2") {
        ...
        $envLines = New-EnvFileContent -Endpoint $azureEndpoint -Key $azureKey -Deployment $azureDeployment -ApiVersion $azureVersion
    ...
    if ($choice -eq "1") {
        ...
        $envLines = New-EnvFileContent -Endpoint "" -Key "" -Deployment "flux-1-1-pro" -ApiVersion "2025-04-01-preview"
        Set-Content -Path ".env" -Value $envLines -Encoding ASCII
        WriteOk ".env template created"
        WriteWarn "Edit .env and add your Azure OpenAI credentials"
    }
}
